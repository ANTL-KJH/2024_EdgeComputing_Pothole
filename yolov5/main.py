        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                if len(det):
                    now = datetime.now()
                    print(type(im0))
                    im0_resized = cv2.resize(im0, (320, 240))
                    ...
                    message = f"{self.GPS.latitude},{self.GPS.longitude},{formatted_time}"
                    self.sock.sendto(message.encode('utf-8'), (self.server_address, self.server_port))
                    self.sock.sendto(im0_resized_jpg, (self.server_address, self.server_port))
                    print(message)
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        if save_csv:  # Write to CSV
                            write_to_csv(p.name, names[int(cls)], conf.item())
                ...
                if view_img:
                    if platform.system() == "Linux" and p not in windows:
                        windows.append(p)
                        cv2.namedWindow(str(p), cv2.WINDOW_NORMAL | cv2.WINDOW_KEEPRATIO)  # show in new window
